...
from retrieval import Retriever, get_embeddings as _get_embeddings, hierarchical_chunks
...
# Bump when the default chunking changes: cached vectors are keyed by content,
# and stale vectors would silently misalign with freshly recomputed chunks.
_EMBED_CACHE_VERSION = "hier1"


def _fixed_mode(split_pattern: str = "") -> bool:
    return bool(split_pattern) or (
        os.getenv("EXTRACTION_CHUNKING", "hierarchical").strip().lower() == "fixed"
    )


def _chunks_for(text: str, split_pattern: str = "") -> list[str]:
    """Chunking for the search tools: structure-aware hierarchical chunks by
    default (same as the deterministic pipeline); an explicit split regex or
    EXTRACTION_CHUNKING=fixed falls back to the legacy fixed-size splitter."""
    if _fixed_mode(split_pattern):
        return _chunk_text(text, split_pattern)
    return hierarchical_chunks(text)


def _embed_cache_key(text: str, split_pattern: str = "") -> str:
    mode = "fixed" if _fixed_mode(split_pattern) else _EMBED_CACHE_VERSION
    return hashlib.sha1(
        (text + "||" + split_pattern + "||" + mode).encode("utf-8")
    ).hexdigest()[:16]


@tool
def search_bm25(paths: list[str], query: str, top_k: int = 5, split_pattern: str = "") -> str:
    """BM25 lexical ranked search across structure-aware chunks of one or more files.
    Args:
        paths: list of virtual paths.
        query: natural-language keyword query.
        top_k: number of top chunks (default 5).
        split_pattern: optional regex to split text into chunks; default = hierarchical section/clause chunks.
    """
    ...
    for vpath, text in _read_files(paths):
        if text.startswith("[ERROR"):
            continue
        for c in _chunks_for(text, split_pattern):
...
# _get_embeddings (incl. the no-credentials egress guard) moved to
# retrieval.get_embeddings; imported at the top for backward compatibility.
...
def search_vector(paths: list[str], query: str, top_k: int = 5, split_pattern: str = "") -> str:
    ...
    Embeddings are cached per-(content,split,chunker) in the persistent cache
    (survives the per-run /scratch wipe), so identical text is embedded only once
    across runs.
    Args:
        paths: list of virtual paths.
        query: search query.
        top_k: number of top chunks (default 5).
        split_pattern: optional regex split; default = hierarchical section/clause chunks.
    """
    import numpy as np
    ...
    for vpath, text in _read_files(paths):
        if text.startswith("[ERROR"):
            continue
        chs = _chunks_for(text, split_pattern)
        if not chs:
            continue
        key = _embed_cache_key(text, split_pattern)
...
# In-process cache of hybrid retrievers keyed by file contents, so repeated
# search_hybrid calls within one run embed the document at most once.
_retriever_cache: dict[str, Retriever] = {}
_RETRIEVER_CACHE_MAX = 8


@tool
def search_hybrid(paths: list[str], query: str, top_k: int = 8) -> str:
    """Hybrid ranked search over structure-aware chunks: BM25 (exact clause
    numbers/terms) fused with semantic vectors via RRF. Use this as the DEFAULT
    search — it beats search_bm25/search_vector alone. Automatically degrades
    to BM25-only when embeddings are unavailable (offline / no key).
    Args:
        paths: list of virtual paths (e.g. ['/scratch/file.md']).
        query: natural-language query; Russian topic keywords work best.
    ...
    for vpath, text in _read_files(paths):
        if text.startswith("[ERROR"):
            continue
        for c in _chunks_for(text):
            all_chunks.append((vpath, c))
        content_hashes.append(hashlib.sha1(text.encode("utf-8")).hexdigest())
    if not all_chunks:
        return "No content to search."

    cache_key = hashlib.sha1(
        ("||".join(content_hashes) + "||" + _EMBED_CACHE_VERSION).encode("utf-8")
    ).hexdigest()[:16]
    retriever = _retriever_cache.get(cache_key)
    if retriever is None:
        retriever = Retriever([c for _, c in all_chunks])
    ...
    idxs = retriever.top_indices(query, top_k, bm25_weight)
    if not idxs:
        return "No matches."
    return "\n\n".join(
        f"--- {all_chunks[i][0]} (hybrid #{rank}) ---\n{all_chunks[i][1]}"
